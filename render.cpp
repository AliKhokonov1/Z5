...
void render(VecPoint cam, VecPoint normal, VecPoint up, double screen, double limit, double alpha, int width, int height, VecPoint light, std::vector<Shape*> &objects){
	...
	VecPoint N;//НОРМАЛЬ К ПОВЕРХНОСТИ
	double t;
	VecPoint L;//НАПРАВЛЕНИЕ ОТ ТОЧКИ ПЕРЕСЕЧЕНИЯ ДО ИСТОЧНИКА СВЕТА
	double NL;//Скалярное произведение И, L
	...
	double color[3];//Чвет пикселя = AMBIENT + DIFFUSE
	double ambient[3];//Фоновая составляющая освещения
	double diffuse[3];//Рассеивающая составляющая освещения

	VecPoint start;
	start = normal * screen + sideway * ((screen_width - pixel_size)/2)+up*((screen_height-pixel_size)/2);//Вектор левого верхнего угла
	...
	for (int i = 0;i < width;i++) {
		for (int j = 0;j < height;j++) {
			ray_direction = start - sideway * pixel_size * (i)-up * pixel_size * (j);//Вектор луча
			intersect = false;
			...
			if (!intersect) {
				//луч не пересёкся ни с кем
				//закрашиваем писель черным
				for (int k = 0; k < 3; k++) {
					image(i, j, k) = 0;
				}
			}
				...
				for (int k = 0; k < 3; k++) {
					diffuse[k] = (NL*(obj->color[k]));//Можно ещё на константу домножить меньше 1 но больше 0
				}
				for (int k = 0; k < 3; k++) {
